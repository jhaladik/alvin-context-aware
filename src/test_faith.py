"""
Comprehensive Test Suite for Faith-Based Evolutionary Agent

Tests all 4 revolutionary components:
...
Usage:
    python test_faith.py checkpoints/faith_evolution_20251119_080256_best_policy.pth
    python test_faith.py <model_path> --episodes 100
    python test_faith.py <model_path> --game pacman --analyze-faith
"""
import sys
import os
...
def test_game_with_faith(agent, observer, game, game_name, num_episodes=50,
    ...
    Args:
        ...
        planning_horizon: Planning lookahead steps
    ...
    for episode in range(num_episodes):
        ...
        while not done and steps < 1000:
            ...
            # Record for analysis
            if oracle:
                # Extract entity distance from observation
                entity_distance = obs[40] if len(obs) > 40 else None
...
def main():
    parser = argparse.ArgumentParser(description='Comprehensive Faith-Based Agent Test')
    ...
    parser.add_argument('--planning-horizon', type=int, default=5, help='Planning horizon')
    ...
    # Load agent
    print(f"{'='*70}")
    print(f"FAITH-BASED EVOLUTIONARY AGENT TEST")
    print(f"{'='*70}")
    ...
    if not args.no_planning:
        ...
        if os.path.exists(world_model_path):
            ...
            if 'state_predictor.0.weight' in state_dict:
                ...
                hidden_dim = 128  # Default for standard observer
            ...
            try:
                ...
                world_model = EntityDiscoveryWorldModel(obs_dim=95, action_dim=4)
                world_model.load_state_dict(state_dict)
                world_model.eval()
                print(f"  Entity Discovery World Model loaded")
            except Exception as e:
                # Fall back to standard world model
                from core.world_model import WorldModelNetwork
                world_model = WorldModelNetwork(state_dim=95, action_dim=4, hidden_dim=hidden_dim)
                world_model.load_state_dict(state_dict)
                world_model.eval()
                print(f"  Standard World Model loaded (hidden={hidden_dim})")
    ...
    if analyze_faith:
        ...
        try:
            from core.entity_discovery import EntityDiscoveryWorldModel
            entity_world_model = EntityDiscoveryWorldModel(obs_dim=95, action_dim=4)
